...
The data to make these decisions correctly already exists.It sits inside Tally, untouched. Tally records what happened. It never tells you why, and never tells you what to do next. Every existing BI tool (Power BI, Zoho Analytics, SAP) is priced and designed for enterprises is too expensive, too complex, and completely blind to the realities of B2B trade in India: 60-day credit cycles, relationship-driven collections, and distributor margins that live or die on a 3% spread.
...
Multi-company support for distributor groups, salesperson ROI module, commodity price signal integration (LME copper and aluminium which are directly relevant to electrical and hardware distributors).
...
The moat is not the technology, it's the position. Once NerveSight's daily brief becomes part of how a distributor starts their morning, we own the decision layer above their data. Every credit call, every inventory purchase, every collections priority runs through us. Switching means losing not just a tool but the operational memory the system has built about their business — which customers are seasonal, which SKUs are structurally low margin, which territory is growing. That memory compounds every day we're live. Tally stores the past. We tell them what to do next. No ERP replaces that.
The second moat is distribution. Indian B2B operates on trust networks where a distributor takes a recommendation from their CA, their trade association, or the person in the same line of business two streets over. We are building inside one of those networks. The first 5 customers are not customers — they are the channel.
...
v0.1 live — runs on sample electrical distributor dataset (7 customers, 7 SKUs, 22 vouchers). Seeking first 5 pilot customers in Mumbai electrical distribution.
------------------------------------------------------------------------------------------------------------------------------------------------------------------------

## About

**Shriman Maheshwari** — Founder  
First-year CS, BITS Pilani (Years 1-2)· AI specialisation, CentraleSupélec Paris (Years 3–4)  
(mailto:swt181@gmail.com)
